...
Main-window layout validation checklist: in the compact player, expand and collapse Activity and Last session in every combination, then trigger the pending snapshot deletion panel and an outside-active-hours warning; each change must resize the window so the last visible row is fully inside the lower border. Open App options and repeat while switching between General and AI configuration and expanding either Advanced section. The window must re-measure visible content, retain its configured flyout anchor, and use its vertical scrollbar only when the display work area limits the measured height.
