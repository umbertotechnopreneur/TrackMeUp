...
- Lets you configure an in-player snapshot interval and edit weekly working hours in a 30-minute grid. The default is Monday-Friday, 09:00-18:00, with a 13:00-14:00 lunch break; weekends are empty. Working hours are stored locally and used to gate scheduled snapshots.
...
- **Snapshot analysis:** scheduled snapshots and other non-deferred captures are analyzed immediately when AI is enabled. A manual player snapshot waits through the 30-second deletion window; if it is deleted, no AI request is made, and if it remains, the exact retained capture is analyzed once the window expires.
...
Screenshot gallery validation checklist: open the gallery twice and confirm that only one window is activated, open it from the `Wayback Machine` flyout entry, select another day with the floating WinUI `CalendarDatePicker`, and verify that a manual capture is labeled `Manuale` while an automatic capture is labeled `Pianificato`. From the title-bar menu, confirm that Save, Share, and Open folder are available before the separated delete actions; with an empty day selected, Save, Share, and Delete must be disabled while Open folder remains available.
