...
Capture deletion validation checklist: take a manual snapshot from the player, confirm that the delete action and 30-second progress countdown appear, delete it before the countdown expires, confirm that no AI request was made, and confirm that the latest-session preview no longer shows it. Repeat without deleting, wait for the countdown to expire, and confirm that the same capture is analyzed exactly once. In the screenshot gallery ellipsis menu, confirm that Delete screenshot removes the image artifacts while Delete snapshot removes the associated local snapshot-analysis record.
...
Player overflow validation checklist: open the overflow menu from the always-visible ellipsis in light and dark themes, confirm that the compact surface contains Reports, Captured moments, App options, OpenAI, screenshot, and About; open Reports and Captured moments and confirm that both land on today by default and still allow filtering afterward; toggle screen capture off and on, reopen the menu, and confirm that the persisted switch state matches the app options page.
...
Scheduling validation checklist: open Snapshot schedule, set an interval, and confirm that the player displays the next-snapshot countdown. In the working-hours grid, select contiguous 30-minute blocks and leave a gap inside them; reopen the dialog and confirm that the selected blocks and the gap are restored as active time and a break. Confirm that snapshots occur only inside selected working hours and never during a configured break. Confirm that the optional screen capture occurs only when enabled and its normal privacy checks pass.
