...
Screenshot gallery validation checklist: retain captures on two different days, restart the app, open the gallery twice and confirm that only one window is activated, then open it from the `Wayback Machine` flyout entry and confirm that the most recent retained day is selected with its captures visible. Select another day with the floating WinUI `CalendarDatePicker`, verify that a manual capture is labeled `Manuale` while an automatic capture is labeled `Pianificato`, and use the App options link to open the configured snapshot and debug-image folder. From the title-bar menu, confirm that Save, Share, and Open folder are available before the separated delete actions; with an empty day selected, Save, Share, Delete, and the details toggle must be disabled while Open folder remains available. In light and dark themes, confirm that the main surface shows one selected screenshot only, with no carousel side previews, and that the screenshot fits without cropping at 100% zoom while the top zoom controls show the current percentage and can inspect the image up to 500%. Confirm that the viewer save icon opens the same export flow as the title-bar Save action. Confirm that the compact metadata row uses rounded translucent chips with icons, labels, values, and an unclipped historical activity index, and remains visually separated from the contained filmstrip. Open the keyboard-accessible details toggle to the right of the ellipsis and confirm that the translucent Mica sidebar follows the selected screenshot, shows app, capture type, origin, span labels, and activity index as compact chips, and renders an available AI description as inert headings, paragraphs, and lists without clickable links or HTML; captures without an analysis must show the explicit empty state. With 0, 1, 2, 6, and at least 500 captures, select screenshots repeatedly from the virtualized filmstrip and confirm synchronized screenshot, metadata, details, selected thumbnail, no image flash, and no growing working set after repeated navigation. Include 16:9, ultrawide/multi-monitor, and portrait captures and confirm every screenshot remains fully visible at reset zoom without cropping or a decorative carousel frame. Confirm that the virtualized filmstrip stays inside its rounded glass surface, keeps the selected thumbnail visible, and exposes enabled left/right paging arrows only when its content overflows. Disable Windows animation effects and confirm that selection and keyboard focus remain visible.
