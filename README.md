...
Search and keyboard shortcuts validation checklist: keep the main window focused, press `Ctrl+Shift+P`, and confirm that the local screenshot search opens with focus in the query field. Move the pointer to each connected monitor and repeat the shortcut, confirming that the search window is centered in the pointer's monitor work area. Open the application menu and confirm that the primary actions show their `Ctrl+Shift+...` shortcuts and invoke the same commands as their menu items.
