...
- [ ] Inspect both hero and compact world clocks: each skyline surface meets the next without a card outline, rounded card edge, gap, or shadow, and its always-visible detail/delete actions have no 1-DIP outline; expand the detail and verify the sun/moon rows and 24-hour daylight arc use the full rail width without extra horizontal padding.
- [ ] With the world-clock rail both hidden and visible, drag the player from the title text/blank caption area and verify every custom title command still clicks at 100%, 125%, 150%, and 200% DPI; on Windows 11 the outer window corners are rounded, the native title bar shows only Close with a 12-DIP gap after TrackMeUp commands, Cancel keeps tracking active, and confirming Close states that tracking will be suspended before shutdown.
