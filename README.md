...
- [ ] Check local search on a narrow display and with Windows text scaling at 200%: the preview stacks below the result list when needed, both panes scroll vertically, and the footer and Open snapshot action remain reachable. During typing, first-index creation, no matches, and errors, status text must remain fully visible above the results/footer. Confirm the thin gradient flows within the input edge without moving the field, pauses when the window is inactive or Windows animations are disabled, and releases its animation on close. The query must not show spelling/prediction popups; selected-result changes must never load screenshot thumbnails or query an AI provider.
...
- [ ] In World Clocks, compare Rome, New York, Sydney, and Ho Chi Minh City in summer/winter and across a DST transition using the time converter. Confirm each clock shows active/inactive DST and, when active, its next end date from installed Windows rules (or explicitly that no end date is available). Resize down: the DST detail should yield to essential content while remaining in the time tooltip and accessible summary. The tooltip's transition time uses the city's offset after the change.
...
- [ ] Search during a long index update: existing results must remain responsive while new captures become searchable after the background updater publishes its next snapshot. First-time index creation may show the existing search progress indicator. More than 2,000 ordinary pending changes must use incremental batches; affected screenshot days must be scanned once per batch and only requested captures should load metadata.
- [ ] After screenshot deletion, analysis deletion, or retention reports success, repeat the query and restart the app: removed content must remain absent. Cancel a rebuild and confirm the last committed index still works; inject a background indexing failure and confirm search reports an error until a successful explicit rebuild. Exit during an update and verify all index files are released.
- [ ] Upgrade from the old derived index and verify that `lucene-v4` contains only source documents, the obsolete indexes are removed, and the rebuild window shows one search index without suggestions. Incomplete test IPC frames must time out and release their bounded slots.
