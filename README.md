...
Dialog validation checklist: start the app with OpenAI enabled and its configured environment variable empty, and confirm that one localized warning Mica window names the variable without exposing a secret. Force a provider failure after a retained frame capture and confirm that the shared runtime delivers one localized error dialog to both in-process and named-pipe UI modes while keeping the screenshot. Verify information, warning, and error accents in light, dark, and high-contrast themes; use a long custom title and message and confirm that the message scrolls without clipping. Open the schedule preset and retention cleanup confirmations, verify that all dialogs are serialized through the same Mica engine, and confirm that Cancel, Alt+F4, or closing the window never applies the destructive action while the explicit primary button does.
