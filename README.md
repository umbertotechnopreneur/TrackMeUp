...
- [ ] **Advanced-sensor installation:** on Windows x64 without PawnIO, install a signed release or select **Install and activate advanced sensors**; verify the bundled official installer requests administrator consent and works offline. An equal/newer PawnIO installation must not be replaced. Declined consent, setup failure, modified installer bytes, and a required Windows restart must remain explicit; closing the sensor window must not terminate or duplicate an ongoing driver installation. After restarting TrackMeUp, activate the collector again without reinstalling the driver or elevating the whole app. Check all ten languages, narrow widths, 200% text scaling, and matching localized tooltip/accessibility text. ARM64 must not bundle or launch the unsupported sensor prerequisite.
...
- [ ] **Sensor background graphs:** verify each activity trace fills its component row behind the labels without a separate chart column or bottom section. At a 476 × 476 logical-pixel track area, three devices must fit on a page. Name, capacity, current value and temperature must stay readable in light/dark themes, narrow windows and at 200% text scaling; high contrast must hide the decorative trace. Missing samples must still split the graph, and battery charge must keep its slim level bar.
