...
$liveBook = Join-Path $root 'Planner_Disponibilita_A4.xlsx'
...
$newSaints = Join-Path $runRoot 'saints-agent\Santi.csv'
$newWeather = Join-Path $runRoot 'weather-agent\Meteo.csv'
$newCities = Join-Path $runRoot 'weather-agent\Citta.csv'
...
$workbookPath = Join-Path $working 'Planner_Disponibilita_A4.xlsx'
Copy-Item -LiteralPath $liveBook -Destination $workbookPath
foreach ($name in @('Periodi_DST.csv','Fasi_lunari.csv')) { Copy-Item -LiteralPath (Join-Path $liveData $name) -Destination (Join-Path $stageData $name) }
Copy-Item -LiteralPath $newCities -Destination (Join-Path $stageData 'Citta.csv')
Copy-Item -LiteralPath (Join-Path $runRoot 'dati_preparati\Calendari.csv') -Destination (Join-Path $stageData 'Calendari.csv')
Copy-Item -LiteralPath $newSaints -Destination (Join-Path $stageData 'Santi.csv')
Copy-Item -LiteralPath $newWeather -Destination (Join-Path $stageData 'Meteo.csv')
$backup = Join-Path $runRoot ('backup-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
New-Item -ItemType Directory -Path $backup | Out-Null
Copy-Item -LiteralPath $liveBook -Destination (Join-Path $backup 'Planner_Disponibilita_A4.xlsx')
# Explicit CSV list: the API key is never copied into backups or reports.
foreach ($name in @('Calendari.csv','Santi.csv','Meteo.csv','Citta.csv')) {
...
let
 ...
 Raw = Csv.Document(File.Contents(Folder & "/Calendari.csv"), [Delimiter=";",Encoding=65001,QuoteStyle=QuoteStyle.Csv]),
...
let
 Folder = Text.TrimEnd(Text.From(Excel.CurrentWorkbook(){[Name="DataFolder"]}[Content]{0}[Column1]), {"/", "\"}),
 Raw = Csv.Document(File.Contents(Folder & "/Citta.csv"),[Delimiter=";",Encoding=65001,QuoteStyle=QuoteStyle.Csv]),
 Headers = Table.PromoteHeaders(Raw,[PromoteAllScalars=true]),
...
'@
    ...
    $settings.Range('A34:M34').Merge();$settings.Range('A34').Value2='Vuota: legge Meteo.csv. Compilata: Dati > Aggiorna tutto interroga le città selezionate (meteo attuale).'
...
let
 Folder = Text.TrimEnd(Text.From(Excel.CurrentWorkbook(){[Name="DataFolder"]}[Content]{0}[Column1]), {"/", "\"}),
 Raw = Csv.Document(File.Contents(Folder & "/Meteo.csv"),[Delimiter=";",Encoding=65001,QuoteStyle=QuoteStyle.Csv]),
 Headers = Table.PromoteHeaders(Raw,[PromoteAllScalars=true]),
...
'@
    ...
    $info.Range('A17:J17').Merge(); $info.Range('A17').Value2 = 'Aggiorna tutto: chiave vuota in Parametri B32 = Meteo.csv; chiave compilata = API OpenWeather per le città selezionate.'
