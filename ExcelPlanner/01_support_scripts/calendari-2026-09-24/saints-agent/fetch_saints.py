...
def build():
    ...
    manifest = {
        "dataset": "Santi.csv", "as_of_date": AS_OF, "language": "it",
    ...
    (ROOT / "Santi.csv").write_bytes(encoded)
    save_json(ROOT / "provenance.json", provenance)
    save_json(ROOT / "manifest.json", manifest)
    print(json.dumps({"output": str(ROOT / "Santi.csv"), "coverage": coverage, "sha256": manifest["output_sha256"]}, ensure_ascii=False))
...
if __name__ == "__main__":
    ...
    actions.add_argument("--build", action="store_true", help="Rebuild Santi.csv and manifests offline from verified raw sources.")
