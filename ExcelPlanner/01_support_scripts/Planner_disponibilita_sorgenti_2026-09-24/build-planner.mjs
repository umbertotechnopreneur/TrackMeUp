...
const target=`${out}/Planner_Disponibilita_A4.xlsx`;
...
await fs.writeFile(`${out}/Calendar data/Calendari.csv`,'\uFEFF'+['Calendar;Date;Name;Active;Source',...data.holidays.map(e=>csvrow([e.calendar,e.date,e.name,e.active,e.source]))].join('\r\n')+'\r\n','utf8');
...
const help=[
 ...
 ['Santi e ricorrenze','Santi.csv è un campione di due ricorrenze, non un elenco completo né il calendario liturgico annuale. Una riga per mese/giorno; più nomi nella stessa cella. Nessun effetto sugli orari. Eventi mobili vanno modellati con date specifiche in una futura versione.'],
