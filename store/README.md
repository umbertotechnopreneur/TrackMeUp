...
Edit [`listing.json`](listing.json) when updating the listing. It keeps the product name, translations, public links, and screenshots together in Git, so we can review each change before it reaches the Store.
...
The listing is available in English (`en-US`), Italian (`it-IT`), French (`fr-FR`), German (`de-DE`), Spanish (`es-ES`), Simplified Chinese (`zh-Hans`), Vietnamese (`vi-VN`), Korean (`ko-KR`), European Portuguese (`pt-PT`), and Brazilian Portuguese (`pt-BR`). It explains that we use TrackMeUp ourselves and that it's a working app. It also covers the MIT-licensed source code, history stored on your PC by default, and optional AI and screenshots.
...
Screenshots aren't enabled in the listing yet. Before the first submission, we need images that show the app working reliably and as people will actually use it.
