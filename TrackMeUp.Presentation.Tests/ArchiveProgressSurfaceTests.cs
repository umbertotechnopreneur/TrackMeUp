...
public sealed class ArchiveProgressSurfaceTests
{
    ...
    public void ArchiveDialog_ProvidesLocalizedProgressAndStopsPollingOnClose()
    {
        ...
        while (root is not null && !File.Exists(Path.Combine(root.FullName, "TrackMeUp.slnx"))) root = root.Parent;
        Assert.NotNull(root);
        var xaml = XDocument.Load(Path.Combine(root.FullName, "TrackMeUp", "OperationProgressDialogWindow.xaml"));
        ...
        var source = File.ReadAllText(Path.Combine(root.FullName, "TrackMeUp", "OperationProgressDialogWindow.xaml.cs"));
        ...
        foreach (var file in Directory.EnumerateFiles(Path.Combine(root.FullName, "TrackMeUp.Core", "Localization"), "*.json"))
        {
