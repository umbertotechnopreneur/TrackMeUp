...
public sealed class MainPlayerResponsiveLayoutTests
{
    ...
    public void Player_HasNoScrollContainerAndFitsBelowTheTitlebar()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "TrackMeUp", "MainWindow.xaml")))
        {
            directory = directory.Parent;
        }

        Assert.NotNull(directory);
        var view = XDocument.Load(Path.Combine(directory.FullName, "TrackMeUp", "MainWindow.xaml"));
