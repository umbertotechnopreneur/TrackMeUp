...
`WorkTrail.Ocr` is an optional, on-device screenshot OCR adapter over
...
The Windows OCR API requires package identity for desktop applications. WorkTrail's
...
- [ ] Integration: run OCR against a retained screenshot from the packaged WorkTrail app on a machine with the selected Windows OCR language installed.
