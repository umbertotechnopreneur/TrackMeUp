...
- Through September 27, 2026 inclusive, focus on maintenance and stability to prepare WorkTrail for publication on the Microsoft Store. Do not add new features during this period.
...
- Keep MailMeUp, PromptMeUp, and WorkTrail visually consistent using [the MeUp style guide](docs/assets/meup/README.md). Use a common README structure and author signature, with a distinct accent color and concrete benefit for each product. Keep the main product purpose ahead of optional extras.
...
- Keep WinUI views, code-behind, Spectre commands, prompts, and renderers passive: they may only collect input, bind/render DTOs, and invoke `IWorkTrailApplication`.
...
- Put application behavior, persistence, OS interop, capture, environment access, HTTP, retention, and startup changes behind `WorkTrail.Core` application services.
...
- `WorkTrail/` for the Windows-first WinUI 3 app (`net10.0-windows10.0.19041.0`, x64/ARM64).
...
- The WorkTrail name, logos, app icons, and project-authored brand artwork are separate from the MIT grant; follow `TRADEMARKS.md` and asset-specific provenance records.
