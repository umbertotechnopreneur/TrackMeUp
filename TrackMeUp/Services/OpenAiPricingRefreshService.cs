...
public sealed class OpenAiPricingRefreshService : IDisposable
{
    ...
    internal async Task<int> RefreshAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var markdown = await Http.GetStringAsync(OpenAiPricingMarkdownParser.PricingMarkdownUrl, cancellationToken).ConfigureAwait(false);
    ...
    private void HandleTimerTick(object? state) => _ = RunScheduledRefreshAsync();

    private async Task RunScheduledRefreshAsync()
    {
        try
        {
            await RefreshIfStaleAsync(CancellationToken.None).ConfigureAwait(false);
        }
