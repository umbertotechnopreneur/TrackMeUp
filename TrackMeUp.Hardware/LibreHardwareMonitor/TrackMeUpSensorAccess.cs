...
public static class TrackMeUpSensorAccess
{
    ...
    public static float? ReadUpdatedValue(ISensor sensor, IReadOnlyDictionary<ISensor, long> versions, bool staticMetadata)
    {
        ...
        return staticMetadata || !versions.TryGetValue(sensor, out var before) || value.TrackMeUpUpdateSequence != before ? sensor.Value : null;
    }
    ...
    private static void Capture(IHardware hardware, Dictionary<ISensor, long> versions)
    {
        foreach (var sensor in hardware.Sensors)
        {
            ...
            if (sensor is Sensor value) versions[sensor] = value.TrackMeUpUpdateSequence;
        }
